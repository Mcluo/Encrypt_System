...
public class AES {
	...
	private int aes_x(int x, int ts) // 模 1 0001 1011 不太懂
	{
		while (ts-- > 0) {
	...
	private int aes_mul(int x, int y) { // 多项式乘法
		return ((((y >> 0) & 1) * aes_x(x, 0)) ^ (((y >> 1) & 1) * aes_x(x, 1)) ^ (((y >> 2) & 1) * aes_x(x, 2))
				^ (((y >> 3) & 1) * aes_x(x, 3)) ^ (((y >> 4) & 1) * aes_x(x, 4)) ^ (((y >> 5) & 1) * aes_x(x, 5))
				^ (((y >> 6) & 1) * aes_x(x, 6)) ^ (((y >> 7) & 1) * aes_x(x, 7)));
	}
	...
	private void inv_byte_replace() { // 逆向字节代换
		for (int i = 0; i < 16; i++) {
			result[i] = inv_Sbox[result[i]];// Sbox[i]表示Sbox中第i个元素
		}
	}
	...
	public String getResult() {
		...
		StringBuilder ss = new StringBuilder();// stringbuilder类是针对string类对象不能修改的缺点而改进产生能修改的特点的类
		int bit;
		for (int i = 0; i < 16; i++) {
			bit = (result[i] & 0xf0) >> 4;
			ss.append(hex[bit]);
