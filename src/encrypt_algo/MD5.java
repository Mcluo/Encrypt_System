...
public class MD5 {
	...
	// 应该是4*64=256位吧,应该是为了避免signed造成的问题
	...
	public long[] digest(byte[] inputBytes) {
...
//				result[i] = result[i] >> 4;
//				temp = result[i] & 0x0FL;
//				resStr += hexs[(int) (temp)] + a;// String中的+号，会被转换为StringBuffer的append()方法来操作的。
//				result[i] = result[i] >> 4;
//			}
//		}
		String C ="";
    	byte[] out = outbyteArray();
    	for(int i=0;i<32;i++)
...
//		return resStr;
	...
	public byte[] outbyteArray() {
		byte[] out = new byte[32];
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 8; j++) {
				out[j + i * 8] = (byte) (result[i] >> (4 * (7-j)) & 0x0FL);
			}
		}
		return out;
	}
